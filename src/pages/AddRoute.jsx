...
export const AddRoute = () => {
  const { addRoute, drivers, vehicles } = useDepot();
  const navigate = useNavigate();

  const [routeName, setRouteName] = useState('');
  const [routeNumber, setRouteNumber] = useState('');
  const [startLocation, setStartLocation] = useState('Colombo Fort');
  const [endLocation, setEndLocation] = useState('Kandy Goods Shed');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-slate-855 dark:text-slate-50">Create New Route</h1>
          <p className="text-xs text-slate-550 dark:text-slate-400 mt-0.5">Register a new transit service line and plot station path nodes.</p>
        </div>
      </div>

      <div className="bg-white dark:bg-darkbg-800 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm p-6 max-w-4xl">
                ...
                {stops.map((stop, idx) => (
                  <div key={idx} className="flex items-center space-x-2 bg-slate-50 dark:bg-darkbg-900 p-2.5 rounded-lg border border-slate-100 dark:border-slate-800">
                    ...
                    <select
                      ...
                      className="flex-1 bg-transparent border-none text-slate-800 dark:text-slate-100 text-sm focus:outline-none"
                    >
                      {cityOptions.map(city => (
                        <option key={city} value={city}>{city}</option>
                      ))}
